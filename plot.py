    ...
    for route in scenario["routes"]:
        ...
        for path in route["route_paths"]:
            for (i, route_section) in enumerate(path["route_sections"]):
                ...
                print("Adding Edge from {} to {} with sequence number {}".format(from_node_id(path, route_section, i),
...
def save_graph(route_graphs):
    for k, route_graph in route_graphs.items():
        ...
        nx.draw(route_graph, pos, edge_color='black', width=1, linewidths=1, node_size=500, node_color='pink',
                alpha=0.9)
