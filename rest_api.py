...
def do_loesung_validation(scenario, solution):
    ...
    print("upload finished with status {upload_response.status_code}")
    ...
    print("validation finished with status {validation_response.status_code}")
...
if __name__ == "__main__":
    ...
    print()
    print("There are {len(warnings)} warnings and {len(errors)} errors" + "\n")

    if len(errors) > 0:
        print("the solution has {len(errors)} errors. It will not be accepted as a feasible solution. "
              "See the error messages for details.")
        ...
        print()
        print("the solution has {len(warnings)} warnings. It will be accepted as a feasible solution. ")
        if validation_result['objective_value'] > 0.0:
            print("However, it will incur {validation_result['objective_value']} penalty points in the grader.")
